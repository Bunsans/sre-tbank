...
class Config(object):
    ...
    )  # Update to typical Kubernetes Prometheus URL
    ...
    def query_range(
        ...
        try:
            ...
            if not content or content.get("status") != "success":
                logging.warning(
                    f"Prometheus query failed or returned no data for query '{query}': {content.get('errorType', 'N/A')} - {content.get('error', 'N/A')}"
                )
                return []
            ...
            logging.error(
                f"Unexpected error processing Prometheus response for '{query}': {error}"
            )
            return []
...
def main():
    ...
    try:
        ...
        while True:
            ...
            api_total = prom.query_range(
                api_total_query,
                start_time,
                current_time,
                step=f"{config.sla_period_days*1440}m",
            )  # Single point at end
            api_success = prom.query_range(
                api_success_query,
                start_time,
                current_time,
                step=f"{config.sla_period_days*1440}m",
            ...
            api_availability_percentage = calculate_availability_sli(
                [[0, api_total_val]], [[0, api_success_val]]
            )
            ...
            logging.info(
                f"API Availability: {api_availability_percentage:.3f}% (SLO: {api_availability_slo}%), Bad: {api_availability_is_bad}"
            )
            ...
            notification_total = prom.query_range(
                notification_total_query,
                start_time,
                current_time,
                step=f"{config.sla_period_days*1440}m",
            )
            notification_success = prom.query_range(
                notification_success_query,
                start_time,
                current_time,
                step=f"{config.sla_period_days*1440}m",
            )

            notification_total_val = (
                float(notification_total[-1][1]) if notification_total else 0
            )
            notification_success_val = (
                float(notification_success[-1][1]) if notification_success else 0
            )

            notification_delivery_percentage = calculate_availability_sli(
                [[0, notification_total_val]], [[0, notification_success_val]]
            )
            ...
            logging.info(
                f"Notification Delivery: {notification_delivery_percentage:.3f}% (SLO: {notification_delivery_slo}%), Bad: {notification_delivery_is_bad}"
            )
