...
PROBER_CREATE_USER_SCENARIO_SUCCESS_FAIL_TOTAL = Counter(
 "prober_create_user_scenario_success_fail_total", "Total count of failed runs the create user scena
)
PROBER_CREATE_USER_SCENARIO_DURATION_SECONDS = Gauge(
 "prober_create_
 user_scenario_duration_seconds", "Duration in secons of runs the create user scenari
)
env = Env()
env.read_env()
class Config(object):
